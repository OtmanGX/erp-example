...
export class OrdersSalepointEffects {
  ...
  loadOrdersSalepoint$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((action) =>
        this.shortFeatureService.getAllOrdersBySalepoint(action.salepoint_id).pipe(
          map((data) =>
            OrdersSalepointActions.loadOrdersSalepointSuccess({ordersSalepoint: data.data.sales_order})
          ),
          catchError((error) => {
            ...
            return of(OrdersSalepointActions.loadOrdersSalepointFailure({ error }));
          })
        )
      )
    )
