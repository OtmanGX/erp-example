...
import {
  Services_items,
  Service_purchases,
} from './customerPurchases/service_purchases.model';
...
export class Delivery extends BaseModel {
  ...
  amountMatPremiere(): void {
    this.totalMatierePremiereAmount = this.itemsMatierePremieres.reduce(
      function (total, item) {
  ...
  amountServices(): void {
    this.totalServicesAmount = this.itemsServices.reduce(function (
      total,
      item
    ) {
      return total + item.total_purchase;
  ...
  amountConsommables(): void {
    this.totalConsommablesAmount = this.itemsConsommables.reduce(function (
      total,
      item
    ) {
      return total + item.total_purchase;
  ...
  amountAccessoires(): void {
    this.totalAccessoiresAmount = this.itemsAccessoires.reduce(function (
      total,
      item
    ) {
      return total + item.total_purchase;
  ...
  amountSystemeApparent(): void {
    this.totalSystemApparentAmount = this.itemsSystemApparent.reduce(function (
      total,
      item
    ) {
      return total + item.total_purchase;
